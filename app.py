...
STUDENT_LIST_FILE = "master_list.csv"    
CLASS_NAME_FILE = "class_info.txt"       
...
# ==========================================
def load_data():
    # 建立一個基礎的空白表格，並強制設定日期格式
    empty_df = pd.DataFrame(columns=["日期", "學生姓名", "狀態", "備註"])
    empty_df['日期'] = pd.to_datetime(empty_df['日期']) 

    if os.path.exists(DATA_FILE):
        try:
            df = pd.read_csv(DATA_FILE, encoding='utf-8-sig')
            # 使用 errors='coerce' 確保髒資料不會讓系統崩潰
...
with tab3:
    ...
    with st.expander("🏫 班級形象設定", expanded=True):
        new_name = st.text_input("輸入您的班級名稱", value=class_name)
        if st.button("更新名稱"):
    ...
    with st.expander("👨‍🎓 學生名冊管理", expanded=False):
        raw_list = st.text_area("在此貼上全班名單 (支援換行或逗號隔開)")
        if st.button("確認更新名冊"):
            ...
            if final_list:
                ...
                st.success(f"已成功建立 {len(final_list)} 位學生名冊！")
                st.rerun()
